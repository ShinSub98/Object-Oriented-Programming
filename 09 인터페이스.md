...
상속 파트의 자동 타입 변환에서 부모-자식 클래스 간 자동 타입 변환이 일어나면
중복되는 메소드는 재정의된 메소드를 사용하고, 자식 클래스에서 새로 선언된 메소드는 사용이 불가한 것처럼

인터페이스 타입으로 자동 타입 변환된 객체는
인터페이스와 객체 둘 다 갖고 있는 메소드가 아니면 사용할 수 없다.
